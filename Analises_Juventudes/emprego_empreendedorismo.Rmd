...
limits = c(20170129,20170306) %>% ymd %>% as_date
...
tipo_empreend = dados[,c(89,8,6,7)]
names(tipo_empreend) = c('Região','Nome do empreendimento', 'Setor econômico', 'Tipo de empreendimento')
...
dificuldades = dados[,48] %>% tolower %>% removePunctuation %>%
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
dificuldades_empreend = dados[,c(89,77:81)]
datatable(dificuldades_empreend)
```

### Potencialidades e possibilidades

```{r, echo=FALSE, message=FALSE, warning=FALSE}
datatable(dados[,c(89,82:84)])
```
