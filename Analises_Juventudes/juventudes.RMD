...
g2 <- ggplot(dados2, aes(dados2$idade))+geom_histogram(fill='#ffd42a', bins = 10)+labs(y="", x="Idade", title="Histogramas")+facet_wrap(~parte1)
g2
```
...
if(is.null(tabela$Var.2)){
  names(tabela)[1] <- c("Mobilizador")
} else{
  names(tabela)[1:2] <- c("Mobilizador", "Não declarado")
}
#Quando nao tem 'Nao declarados'

#Quando tem coluna de 'Nao declarados' usar
#names(tabela)[1:2] <- c("Mobilizador", "Não declarado")
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqEstadoCivil<-freq(dados2$`1.9 | Estado Civil`, plot = F) %>% as.data.frame()
freqEstadoCivil
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqReligiao<-freq(dados2$`1.10 | Religião`, plot = F) %>% as.data.frame()
#freqIdade
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqInternetCasa<-freq(dados2$`1.14 | Você tem acesso a internet em sua casa?`, plot = F) %>% as.data.frame()
#freqIdade
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqInternetCelular<-freq(dados2$`1.15 | Você tem acesso a internet pelo celular?`, plot = F) %>% as.data.frame()
#freqIdade
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqFilhos<-freq(dados2$`2.3 | Você tem filhos?`, plot = F) %>% as.data.frame()
#freqIdade
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqCasa<-freq(dados2$`2.5 | Quantas pessoas moram em sua casa?`, plot = F) %>% as.data.frame()
#freqIdade
datatable(freqCasa)
```
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqChefesFamilia<-freq(dados2$`2.6 | Você é chefe de família?`, plot = F) %>% as.data.frame()
#freqIdade
...
```{r, message=F, warning=FALSE,echo=FALSE}
freqEscola<-freq(dados2$`4.2 | Frequenta a escola?`, plot = F) %>% as.data.frame()
#freqIdade
